        ...
        @foreach($posts as $post)
            ...
            <div class="content-post">
                <header class="container-flex space-between">
                    <div class="date">
                        <span class="c-gris-2">{{ $post->published_at->format('M d') }} / {{$post->owner->name}}</span>
                    </div>
                ...
                <footer class="container-flex space-between">
                    <div class="read-more">
                        <a href="{{url('blog/'.$post->url)}}" class="text-uppercase c-green">read more</a>
                    </div>
                    <div class="tags container-flex">
                        @foreach($post->tags as $tag)
                            <span class="tag c-gris-2 text-capitalize"><a href="{{ route('tags.show',$tag) }}">#{{$tag->name}}</a></span>
                        @endforeach
                    </div>
                </footer>
            </div>
